...
document.addEventListener('keydown', function(event) {
    ...
    if(currCell_x >= 0 && currCell_x <= 8 && currCell_y >= 0 && currCell_y <= 8) {
        if(!isNaN(parseInt(val))) {
            oTable.rows[currCell_y].cells[currCell_x].getElementsByTagName("textarea")[0].value = parseInt(val);
...
function build_board() {
    ...
    for (i = 0; i < rowLength; i++){
        ...
        for(var j = 0; j < cellLength; j++){
            ...
            var cellVal = oCells.item(j).getElementsByTagName("textarea")[0].value;
            if(cellVal !== "") {
...
function clear_board() {
    ...
    for (i = 0; i < rowLength; i++){
        ...
        for(var j = 0; j < cellLength; j++){
            ...
            oCells.item(j).getElementsByTagName("textarea")[0].value = "";
        }
    }
}
...
function inject_board(board) {
    ...
    for (i = 0; i < rowLength; i++){
        ...
        for(var j = 0; j < cellLength; j++){
            ...
            oCells.item(j).getElementsByTagName("textarea")[0].value = board[i][j];
        }
    }
}
